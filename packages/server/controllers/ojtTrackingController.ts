...
const getDutyLogId = (value: string | string[] | undefined): number | null => {
  ...
  const dutyLogId = Number.parseInt(idValue, 10);
  return Number.isNaN(dutyLogId) ? null : dutyLogId;
};
...
export const updateDutyLog = async (
  ...
  try {
    ...
    const dutyLogId = getDutyLogId(req.params.id);
    if (dutyLogId === null) {
      res
        .status(400)
